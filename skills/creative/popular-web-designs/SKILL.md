...
**IMPORTANT**: When building dashboard layouts with sidebars, see `references/nextjs-tailwind-layout.md` for the correct `h-screen` + `overflow-hidden` + `min-h-0` pattern. Getting this wrong cuts off scrollable content silently.
...
**This user's standard**: When asked to improve UI/UX or make something "look professional," the default reference is Linear/Shopify — not generic dark themes. Generic "slate-900" palettes with "slate-700" borders are NOT acceptable. The user explicitly rejected "not very intuitive design" and "doesn't look professional like shopify or spotify."

**Minimum bar for dark UIs**:
- Background: `#09090b` or `#08090a` (near-black, not gray-blue slate)
- Borders: `#27272a` or `rgba(255,255,255,0.08)` (subtle, never heavy solid lines)
- Text: `#fafafa` primary, `#71717a` muted (never pure white/black)
- Accent: `#3b82f6` or brand color (used sparingly on interactive elements only)
- Cards: `#18181b` with hover states that lighten slightly
- No gradients on large surfaces unless intentional (avoid `from-slate-800 via-slate-700 to-slate-900` fills)

**If unsure, load `templates/linear.app.md` or `templates/shopify.md`** — these are the reference points for this user's expectations.
